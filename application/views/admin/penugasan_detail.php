    ...
    <div class="card-body">
        <form action="<?= base_url('admin/tambah_penugasan_satu/' . $pesanan->id_pesanan) ?>" method="post">
            ...
            <div class="form-group">
                ...
                <select name="select_tim" id="select_tim" class="form-control">
                    <?php
                        ...
                        foreach ($tim as $t) {
                    ?>
                        <option value="<?= $t->nama_tim?>"><?= $t->nama_tim?></option>
                    <?php } ?>
                </select>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <textarea name="catatan_kerja" id="catatan_kerja" cols="100" rows="10" class="form-control" required><?php echo isset($pesanan->catatan_kerja) ? $pesanan->catatan_kerja : ''; ?></textarea>
            </div>
            <a href="<?php echo base_url('admin/tabel_penugasan_pegawai'); ?>" class="btn btn-warning">Kembali</a>
        </form>
    </div>
    <!-- /.card-body -->
</div>
